...
@pytest.mark.parametrize("get_config", get_test_configs())
def test_hf_casual_fwd_bwd(get_config):
    if not torch.distributed.is_initialized():
        initialize_global_process_group()

    context = contextlib.nullcontext() if get_config.is_valid else pytest.raises(AssertionError)
    with context:
        world_size = torch.distributed.get_world_size()
        _hf_casual_fwd_bwd(get_config.config, get_config.sp_size, world_size // get_config.sp_size)
