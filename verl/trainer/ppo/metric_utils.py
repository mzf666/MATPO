...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - critic/score/accuracy_reward/mean, max, min: Statistics about accuracy reward component
            - critic/score/tool_format_reward/mean, max, min: Statistics about tool format reward component
    ...
    # Add separate reward component metrics if available in non_tensor_batch
    if hasattr(batch, 'non_tensor_batch') and batch.non_tensor_batch:
        ...
        if "accuracy_reward" in batch.non_tensor_batch:
            accuracy_rewards = torch.tensor(batch.non_tensor_batch["accuracy_reward"], dtype=torch.float32)
            metrics.update({
                "critic/score/accuracy_reward/mean": torch.mean(accuracy_rewards).detach().item(),
                "critic/score/accuracy_reward/max": torch.max(accuracy_rewards).detach().item(),
                "critic/score/accuracy_reward/min": torch.min(accuracy_rewards).detach().item(),
            })
        
        # Check for tool_format_reward component (tool_format_reward)
        if "tool_format_reward" in batch.non_tensor_batch:
            tool_format_rewards = torch.tensor(batch.non_tensor_batch["tool_format_reward"], dtype=torch.float32)
            metrics.update({
                "critic/score/tool_format_reward/mean": torch.mean(tool_format_rewards).detach().item(),
                "critic/score/tool_format_reward/max": torch.max(tool_format_rewards).detach().item(),
                "critic/score/tool_format_reward/min": torch.min(tool_format_rewards).detach().item(),
            })
