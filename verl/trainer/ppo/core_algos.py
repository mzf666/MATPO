...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, num_valid_seqs: int = None):
    """
    ...
    bs = loss_mat.shape[0]
    num_valid_seqs = bs if num_valid_seqs is None else num_valid_seqs
    ...
    if loss_agg_mode == "token-mean":
        ...
        # for multi-agent setting, we need to normalize the loss by the number of total sequences for seq-mean
        loss = loss * bs / max(1, num_valid_seqs)
...
def compute_policy_loss(
    ...
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, num_valid_seqs=num_valid_seqs)
